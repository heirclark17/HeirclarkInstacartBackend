...
const foodPreferencesSchema = z.object({
  ...
  favoriteVegetables: z.array(z.string()).min(1),
  favoriteStarches: z.array(z.string()).min(1),
  hatedFoods: z.string().max(200),
  cheatDays: z.array(z.string()).optional(),
...
async function upsertFoodPreferences(
  ...
  const result = await pool.query(
    ...
    DO UPDATE SET
      ...
      favorite_vegetables = EXCLUDED.favorite_vegetables,
      favorite_starches = EXCLUDED.favorite_starches,
    ...
    [
      ...
      data.favoriteVegetables,
      data.favoriteStarches,
      data.hatedFoods,
      data.cheatDays || [],
      data.favoriteSnacks,
      data.mealDiversity,
